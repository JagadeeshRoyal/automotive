...
public class LoginController {
	...
	public ResponseEntity<?> updateProfile(@RequestBody Login request) {
		log.info("Updating profile for email: {}", request.getEmail());
		try {
			 LoginResponse response = loginServiceImpl.updateProfile(request);
			 return ResponseEntity.ok().body(response);
		}catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("failed to update the profile" + request.getEmail());
		}
	}
